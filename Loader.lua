-- VIP Loader System with Auto Detection & Web Key Validation
-- Updated to use https://astrion-keycrate.vercel.app for key validation
...
-- Session storage (in-memory, per game session)
local ValidatedKeys = {}
...
local function validateKey(key)
    if not key or type(key) ~= "string" then return false, "Invalid key" end
    ...
    local decoded, jsonErr = HttpService:JSONDecode(response)
    if not decoded then
...
local function main()
    ...
    -- Session key check (for non-VIP)
    local sessionKey = "key_valid_for_" .. userId
    local alreadyValidated = ValidatedKeys[sessionKey]

    if isVIP or alreadyValidated then
        task.wait(0.5)
    ...
    VerifyButton.MouseButton1Click:Connect(function()
        ...
        task.spawn(function()
            local isValid, errMsg = validateKey(key)
            ...
            if isValid then
                ...
                showStatus(StatusText, "✗ " .. (errMsg or "Invalid key"), false)
            end
        end)
    end)
